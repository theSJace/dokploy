...
export const scanRepository = async (repoPath: string): Promise<ScanResult> => {
	...
	// Docker-first checks
	// -------------------------------------------------------------------------
	if (fileNames.includes("docker-compose.yml") || fileNames.includes("compose.yml")) {
	...
	// -------------------------------------------------------------------------
	if (fileNames.includes("config.toml") || fileNames.includes("hugo.toml") || fileNames.includes("config.yaml")) {
		if (fileNames.includes("content") || fileNames.includes("themes") || fileNames.includes("layouts")) {
			signals.push("Found Hugo project structure (config.toml + content/themes/layouts)");
			return {
				detectedType: "hugo",
				recommendedBuildType: "nixpacks",
	...
	if (deps["next"]) {
		...
		for (const cfgFile of nextConfigFiles) {
			if (fileNames.includes(cfgFile)) {
				try {
					const cfgContent = await readFile(path.join(repoPath, cfgFile), "utf-8");
	...
	if (deps["astro"]) {
		...
		for (const cfgFile of astroConfigFiles) {
			if (fileNames.includes(cfgFile)) {
				try {
					const cfgContent = await readFile(path.join(repoPath, cfgFile), "utf-8");
					if (cfgContent.includes("@astrojs/node") || cfgContent.includes("@astrojs/vercel") || cfgContent.includes("@astrojs/netlify")) {
						isStaticAstro = false;
	...
	if (hasBuildScript) {
		...
		const outDirHint = scripts.build?.match(/(?:--outDir|--out-dir|--output|--dest)\s+(\S+)/)?.[1];
		const likelyStatic = !!(deps["express"] === undefined && !deps["fastify"] && !deps["koa"] && !deps["hapi"]);
		return {
	...
	signals.push("Found package.json without recognisable framework or build script");
	return {
		detectedType: "node_app",
		recommendedBuildType: "nixpacks",
		recommendedDeploymentTarget: "server",
