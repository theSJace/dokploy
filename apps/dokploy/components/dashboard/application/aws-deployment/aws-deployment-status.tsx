...
export const AwsDeploymentStatus = ({ applicationId }: Props) => {
	...
	const status =
		statusConfig[config.status as keyof typeof statusConfig] ||
			...
			<CardContent className="space-y-4">
				...
				{config.status === "active" && config.subdomain && (
					<p className="text-xs text-muted-foreground">
						DNS changes may take up to 60 seconds to propagate. CloudFront
						distributions can take 5–15 minutes to fully deploy.
					</p>
				)}
