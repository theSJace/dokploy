...
export const awsDeploymentRouter = createTRPCRouter({
	...
	configure: adminProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const existing = await findAwsDeploymentByApplicationId(input.applicationId);
			if (existing) {
	...
	provision: adminProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const config = await findAwsDeploymentByApplicationId(input.applicationId);
			if (!config) {
				throw new TRPCError({
					code: "NOT_FOUND",
	...
	scanRepository: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const repoRoot = application.buildType === "dockerfile"
